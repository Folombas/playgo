...
func (ts *Tileset) createIronOreTile() *Sprite {
	...
	// Iron deposits (brownish-orange spots)
...
func (ts *Tileset) createGoldOreTile() *Sprite {
	...
	// Gold deposits (sparkly yellow spots)
...
func (ts *Tileset) createDiamondOreTile() *Sprite {
	...
	// Diamond deposits (bright cyan crystals)
...
func (ts *Tileset) createPlankTile() *Sprite {
	...
	// Plank pattern (horizontal boards)
...
func (ts *Tileset) createCraftingTableTile() *Sprite {
	...
	// Tool pattern on top (simplified hammer shape)
	// Hammer head
	for y := 8; y < 18; y++ {
		for x := 12; x < 28; x++ {
	...
	for x := startX; x < endX && x < g.world.width; x++ {
		for y := startY; y < endY && y < g.world.height; y++ {
			...
			if block.typ != Air {
				...
				// Use tileset texture
				g.tileset.DrawTile(screen, block.typ, drawX, drawY)
			}
		}
	}
}
